...
func button(parent:Control,text:String,rect:Rect2,callback:Callable,primary:bool=false) -> Button:
 var b=Button.new();b.text=text;b.position=rect.position;b.size=rect.size;b.focus_mode=Control.FOCUS_NONE;b.add_theme_font_size_override("font_size",22)
...
func create_stick():
 ...
 stick.position=Vector2(28,415) if sim.mode=="home" else Vector2(25,444)
 stick.size=Vector2(177,177);hud.add_child(stick)
func update_hud():
 Hud.update(self)
...
func open_save_tools():
 ...
 label(p,progress.warning if progress.write_blocked else "Es gibt kein Benutzerkonto und keinen Cloud-Save.",Rect2(29,317,800,60),20,GOLD).autowrap_mode=TextServer.AUTOWRAP_WORD_SMART
...
func export_save():
 if OS.has_feature("web"):
  var text=JSON.stringify(progress.data,"  ")
  ...
  if f:f.store_string(JSON.stringify(progress.data,"  "));f.close();toast("Sicherung: "+ProjectSettings.globalize_path(path))
...
func prepare_save_import(text:String):
 ...
 if not raw is Dictionary or int(raw.get("version",0)) not in [2,3,4,5,6,7]:toast("Ungültige oder neuere Sicherung; nichts wurde verändert.");return
