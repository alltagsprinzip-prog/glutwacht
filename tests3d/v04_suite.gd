...
func _initialize():
 ...
 check(q.data.version==4 and q.data.hero=="ninja" and q.data.wins==8 and q.data.hall==3 and q.data.jobs.is_empty(),"v3 migration preserves village without invented jobs")
 ...
 check(after.army_target(after.allies[0]).uid=="t0","first target overrides default nearest preference")
 after.first_target="";after.priority="resources"
 check(after.army_target(after.allies[0]).kind in ["lumber","goldmine"],"resource priority targets actual resource buildings")
 ...
 print("V04_TESTS ",count-failed,"/",count);quit(1 if failed else 0)
